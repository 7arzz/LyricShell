...
function generatePythonScript(base64Audio, title, artist, parsedLyrics, duration, audioExt) {
  ...
  const py = [
    ...
    '                pass',
    '        ',
    '        # 2. Try Windows MCI (winmm) for Windows systems (perfect native fallback for M4A/AAC/MP3)',
    '        if sys.platform == "win32":',
    '            try:',
    '                buf = ctypes.create_unicode_buffer(260)',
    '                ctypes.windll.kernel32.GetShortPathNameW(self.file_path, buf, 260)',
    '                self.short_path = buf.value',
    '                ',
    '                res = ctypes.windll.winmm.mciSendStringW(f"open {self.short_path} alias my_music", None, 0, 0)',
    '                if res == 0:',
    '                    self.backend = "mci"',
    '                    length_buf = ctypes.create_unicode_buffer(128)',
    '                    ctypes.windll.winmm.mciSendStringW("status my_music length", length_buf, 128, 0)',
    '                    try:',
    '                        self.duration = float(length_buf.value) / 1000.0',
    '                    except:',
    '                        self.duration = DURATION',
    '                    ctypes.windll.winmm.mciSendStringW("close my_music", None, 0, 0)',
    '                    return',
    '            except:',
    '                pass',
    '        ',
    '        # 3. macOS afplay (built-in, supports M4A/AAC/MP3 natively)',
    ...
    '        elif self.backend == "mci":',
    '            ctypes.windll.winmm.mciSendStringW(f"open {self.short_path} alias my_music", None, 0, 0)',
    '            ctypes.windll.winmm.mciSendStringW("play my_music", None, 0, 0)',
    ...
    '            ctypes.windll.winmm.mciSendStringW("status my_music mode", status_buf, 128, 0)',
    '            return status_buf.value == "playing"',
    '        elif self.backend in ["afplay", "paplay", "aplay", "cvlc", "play", "mpg123"]:',
    '            if self.proc:',
    ...
    '                pass',
    '        elif self.backend == "mci":',
    '            ctypes.windll.winmm.mciSendStringW("stop my_music", None, 0, 0)',
    '            ctypes.windll.winmm.mciSendStringW("close my_music", None, 0, 0)',
    '        elif self.backend in ["afplay", "paplay", "aplay", "cvlc", "play", "mpg123"]:',
    '            if self.proc:',
    '                try:',
